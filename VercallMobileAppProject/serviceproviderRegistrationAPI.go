...
func serviceProviderRegistration(w http.ResponseWriter, r *http.Request){

	//location, err := time.LoadLocation("Australia/Melbourne")
	reqBody, _ := ioutil.ReadAll(r.Body)
	var serviceProviderDetails ServiceProviderRegistration
	json.Unmarshal(reqBody, &serviceProviderDetails)
	...
	params := (&auth.UserToCreate{}).
		...
		Password(serviceProviderDetails.Password).
		DisplayName(serviceProviderDetails.Firstname)
	u, err := authClient.CreateUser(ctx, params)
	if err != nil {
		log.Fatalf("error creating user: %v\n", err)
	}
	log.Printf("Successfully created user: %v\n", u)
	generateEmailVerificationLinkForServiceProvider(ctx, authClient, serviceProviderDetails)
	...
	dateOfBirth, _ := time.Parse("02-01-2006", serviceProviderDetails.DateOfBirth)
	fmt.Println(serviceProviderDetails.Email)
	_, _, err = dbClient.Collection("customer_registration").Add(context.Background(), map[string]interface{}{
		...
		"city": serviceProviderDetails.City,
		"date_of_birth": dateOfBirth,
		"post_code": serviceProviderDetails.PostCode,
		"phone_number": serviceProviderDetails.PhoneNumber,
